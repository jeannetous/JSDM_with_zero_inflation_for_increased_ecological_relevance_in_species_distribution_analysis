...
```{r}
log_abundance <- log(1 + NR_DATA$Abundance)
log_abundance_sorted <- log_abundance[order(NR_DATA$stream), ]
...
pheatmap(log_abundance_sorted,
         ...
         annotation_row = data.frame(Stream = NR_DATA$stream),
         show_rownames = FALSE)
```
...
K = 5
set.seed(1)
folds    = split(sample(1:nrow(covar_data)), rep(1:K, length = nrow(covar_data)))
```
...
covar_data <- NR_DATA %>% select(c("PC1", "PC2", "disturbance", "stream"))
```
## BIC
